class PaymentOrder < ApplicationRecord
  ...
  def for_check_amount
    if (same_payment_amount.to_d + self.check_amount.to_d) > self.payment.total_amount.to_d
      self.errors.add(:check_amount, 'The Amount Large than the Total Payment')
    end

    if (same_order_amount.to_d + self.check_amount.to_d) > self.entity.amount.to_d
